...
class Experiment():
    """ This class construct the C-DOT and CA-DOT experiments for ImageNet dataset"""
    def __init__(self,datasets,graphType,top_rank_r,number_of_nodes,node_i,iterations=100,T_consensus_init=1,T_consensus_max=50,Tc_inc=1):
    ...
    def load_local_data(self):#name of the file, id of the current node
        ...
        """
        data0=pd.read_pickle("Data/Test/{}/data{}.pickle".format(self.datasets,self.node_i))
        data=data0[:1024,:] # 1024 is the dimension for every image in this experiment you can change to other value if you resize the image
        ...
        top_r_eigenvec=pd.read_pickle('Data/Test/{}/Eig_vec_top{}.pickle'.format(self.datasets,self.n))
        self.top_r_eigenvec=top_r_eigenvec[:,:self.r]

        w=pd.read_pickle('Data/Test/{}/weight{}_{}.pickle'.format(self.datasets,self.n,self.topology))
        self.W=w[self.node_i,:]
        
        del data0, data, w, top_r_eigenvec, X_init
    ...
    def ColumnDistributedOI(self):
        ...
        if self.tc_type=="inc":
            folder_name='{}_n{}r{}{}_{}{}'.format(self.datasets,self.n,self.r,self.topology,self.tc_type,self.tc_inc)
        else:
